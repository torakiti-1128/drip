...
- ローカルで先に分解する場合は連番のローカル ID を採番してよい
- GitHub 起票後は `github_issue` と `github_url` を更新し、必要なら同じ番号へ揃える
- Issue ドキュメント名は `<issue-number>-<slug>.md`
- 起票前の一時下書きだけ `draft-<slug>.md` を許可する

例:

```text
docs/11_ISSUE_MANAGEMENT/issues/1-auth-login-flow.md
docs/11_ISSUE_MANAGEMENT/archive/1-auth-login-flow.md
```
