...
- `(#issue)` は必須。実装やドキュメント変更を始める前に対応する GitHub Issue を作成し、件名末尾にその番号を付ける。
...
```bash
# ✅ Good
feat(gemini): add exponential backoff for API timeout retry(#42)
fix(frontend): resolve quiz answer state lost on back navigation(#57)
test(backend): add schema validation edge cases for empty Gemini response(#61)
refactor(backend): extract prompt building logic to PromptTemplateService(#73)
docs(api): add request/response examples for POST /api/v1/reports(#88)
...
7. **Issue Doc** （`docs/11_ISSUE_MANAGEMENT/issues/<issue-number>-<slug>.md`）
