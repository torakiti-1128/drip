...
```
feature/<短い説明>                例: feature/quiz-retry-logic
fix/<短い説明>                    例: fix/gemini-timeout-crash
...
docs(api): add GraphQL request examples for dailyReports
...
- チケット起票は必須にしない。
- 変更の背景と判断理由は、必要な設計書更新かコミット本文に残す。
- 学習目的のスパイク実装でも、型安全性とテストは維持する。
